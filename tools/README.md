...
- `bench.py`: `read_table` / `extract_item_records` / `filter_records_to_last_sample` / `calculate_results_vectorized` / `payload_with_items` / `json.dumps` / 캐시 쓰기 구간별 소요 시간(중앙값, `--repeat` 기본 3)과 payload 크기·항목 수·detail 행 수를 출력. 기본은 데이터 중 가장 큰 조합을 자동 선택하며, `--device --ver --type --lot --purpose --item --readout --ft-temp`로 특정 조합을 지정할 수도 있다.
